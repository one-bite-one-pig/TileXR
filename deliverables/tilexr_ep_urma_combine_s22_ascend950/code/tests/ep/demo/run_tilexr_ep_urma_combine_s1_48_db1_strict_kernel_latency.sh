...
if [[ "${variant}" != "${EXPECTED_VARIANT}" ]]; then
    echo "strict-kernel latency mode requires TILEXR_COMBINE_BUILD_VARIANT=${EXPECTED_VARIANT}" >&2
    exit 2
fi
if [[ -n "${TILEXR_DEMO_PROFILE_DIR:-}" && "${TILEXR_DEMO_PROFILE_DIR:-}" != "-" ]]; then
    echo "strict-kernel latency mode does not allow TILEXR_DEMO_PROFILE_DIR" >&2
    exit 2
fi
...
export TILEXR_DEMO_RUN_ID="${TILEXR_DEMO_RUN_ID:-ep-combine-64rank-bs128-k8-h7168-s1-p48-s16-db1-strict-cycle-v1-$(date +%Y%m%d-%H%M%S)}"
